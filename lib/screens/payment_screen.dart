...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void _processPayment() async {
    ...
    final currentQR = qrProvider.currentQR;

    // Simulate payment processing delay
    await Future.delayed(const Duration(seconds: 2));

    // Activate the QR after successful payment
    if (currentQR != null) {
      await qrProvider.activateQR(
        currentQR['id'] as String,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              // Show QR ID
              if (qrProvider.currentQR != null)
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                  decoration: BoxDecoration(color: AppColors.accent.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
                  child: Text('QR ID: ${qrProvider.currentQR!['uniqueCode'] ?? qrProvider.currentQR!['id']}',
                      style: GoogleFonts.spaceGrotesk(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.accent)),
                ),
              const SizedBox(height: 24),
              SizedBox(width: double.infinity, height: 52, child: ElevatedButton(
                onPressed: () {
                  qrProvider.reset(); // Clear canvas state
                  Navigator.pop(ctx);
              ...
              SizedBox(width: double.infinity, height: 44, child: TextButton(
                onPressed: () {
                  Navigator.pop(ctx);
                  Navigator.pushReplacementNamed(context, '/tag-details');
                },
              ...
              return FadeInUp(delay: Duration(milliseconds: 300 + i * 80), child: GestureDetector(
                ...
                child: Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(16),
                    decoration: BoxDecoration(color: isDark ? AppColors.darkCard : AppColors.lightCard, borderRadius: BorderRadius.circular(14),
                        border: Border.all(color: isSelected ? AppColors.accent : (isDark ? AppColors.darkDivider : AppColors.lightDivider), width: isSelected ? 2 : 1)),
                    child: Row(children: [
                      ...
                      Container(width: 22, height: 22, decoration: BoxDecoration(shape: BoxShape.circle,
                          ...
                          child: isSelected ? Center(child: Container(width: 10, height: 10,
                              decoration: const BoxDecoration(shape: BoxShape.circle, color: AppColors.accent))) : null),
                    ])),
              ));
            }),
          ],
